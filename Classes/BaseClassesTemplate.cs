...
public class BusinessLogicLayerTemplate_5G_CSHARP : ITemplate
{
    ...
    private void BuildDataHandlerBaseNET40()
    {
        ...
        output.AppendLine("    /// <returns></returns>");
        output.AppendLine("    /// <remarks></remarks>");
        output.AppendLine("    internal virtual bool EndTransaction(bool Commit)");
        output.AppendLine("    {");
        output.AppendLine("        try");
        output.AppendLine("        {");
        output.AppendLine("            if (_connection == null)");
    ...
    private void BuildDataHandler()
    {
        ...
        output.AppendLine("    /// <returns></returns>");
        output.AppendLine("    /// <remarks></remarks>");
        output.AppendLine("    public new bool EndTransaction(bool Commit)");
        output.AppendLine("    {");
        output.AppendLine("        try");
        output.AppendLine("        {");
        output.AppendLine("            if (_connection == null)");
    ...
    private void BuildInterfaces()
    {
        ...
        GetHeaderInfo(output);
        output.AppendLine("using System.Data;");
        output.AppendLine("public interface IDataHandler{ IDbTransaction GetTransaction(); }");
    ...
    private void BuildParameterBase(string _namespace)
    {
        ...
        output.AppendLine("    public ParameterItem(String fullQuery)");
        output.AppendLine("	{");
        output.AppendLine("		FullQuery = fullQuery;");
        output.AppendLine("	}");
        output.AppendLine("    public ParameterItem(string fullQuery, string column, string operand, string name, List<ParameterItemValue> values)");
        output.AppendLine("	{");
            ...
            if (entity.Selected)
            {
                ...
                foreach (MyMeta.IColumn column in entity.Columns)
                {
                    ...
                    if (column.IsInForeignKey && !column.IsInPrimaryKey)
                    {
                        ...
                        output.AppendLine("             internal " + column.LanguageType + " _" + GetFormattedEntityName(column.Name) + " { get; set; }");

                        output.AppendLine("             [PropertyAttribute(PropertyAttribute.PropertyAttributeEnum.Fk)] //Is Foreign Key");
                        output.AppendLine("             [PropertyAttributeForeignKeyObjectName(" + System.Convert.ToChar(34) + column.ForeignKeys[0].PrimaryTable.Name + System.Convert.ToChar(34) + ")]// Object name in Database");
                        output.AppendLine("             public " + _namespace + ".Entities.Relations." + GetSchemaName(column.ForeignKeys[0].PrimaryTable.Schema) + "." + column.ForeignKeys[0].PrimaryTable.Name + " " + column.Name);
                        output.AppendLine("             {");
                        output.AppendLine("                 get {");
                        ...
                        output.AppendLine("                             " + GetFormattedEntityName(column.Name) + " = new " + _namespace + ".Business.Relations." + GetSchemaName(column.ForeignKeys[0].PrimaryTable.Schema) + "." + column.ForeignKeys[0].PrimaryTable.Name + "().Items(this._" + GetFormattedEntityName(column.Name) + ").FirstOrDefault();");
                        output.AppendLine("                         }");
